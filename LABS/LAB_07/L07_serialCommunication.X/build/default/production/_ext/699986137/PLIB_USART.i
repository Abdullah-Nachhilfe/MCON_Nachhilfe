...
char Read1USART(void) {
    ...
    if (RCSTA1bits.RX9)
    {
        USART1_Status.RX_NINE = 0;
        if (RCSTA1bits.RX9D)
            USART1_Status.RX_NINE = 1;
    }

    if (RCSTA1bits.FERR)
        USART1_Status.FRAME_ERROR = 1;

    if (RCSTA1bits.OERR)
        USART1_Status.OVERRUN_ERROR = 1;
